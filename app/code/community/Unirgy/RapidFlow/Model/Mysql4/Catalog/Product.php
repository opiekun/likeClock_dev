    ...
    extends Unirgy_RapidFlow_Model_Mysql4_Catalog_Product_Abstract
{
    ...
    public function export()
    {
        ...
        $this->_isAdminStoreId = $profile->getData('store_id')==0;
...
#var_dump($this->_newData);
            ...
            foreach ($row as $col => $v) {
                ...
                foreach ($_kk as $k) {
                    ...
                    } elseif ($v === array('#EMPTY#') || $emptyValueStrategy === 'empty' && $v === array('')) {
                        $this->_newData[$sku][$k] = array('');
                        unset($this->_defaultUsed[$sku][$k]);
                    } elseif ($v === '#DEFAULT#' || $emptyValueStrategy === 'default' && $v === '') {
                        $this->_newData[$sku][$k] = !empty($this->_newDataTemplate[$k]) ? $this->_newDataTemplate[$k] : '';
                        unset($this->_defaultUsed[$sku][$k]);
                    } elseif ($v === array('#DEFAULT#') || $emptyValueStrategy === 'default' && $v === array('')) {
                        $this->_newData[$sku][$k] = !empty($this->_newDataTemplate[$k]) ? array($this->_newDataTemplate[$k]) : array('');
                        unset($this->_defaultUsed[$sku][$k]);
